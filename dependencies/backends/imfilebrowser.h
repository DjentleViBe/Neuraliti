...
namespace ImGui
{
    ...
    public:
        ...
        void SetTitle(std::string title, std::string dirname);
        ...
        bool SetPwd(const std::__fs::filesystem::path &pwd =
                                    std::__fs::filesystem::current_path());

        // get current browsing directory
        const std::__fs::filesystem::path &GetPwd() const noexcept;
    ...
    private:
        ...
        {
            bool                  isDir = false;
            std::__fs::filesystem::path name;
            std::string           showName;
            std::__fs::filesystem::path extension;
        };
        ...
        void SetPwdUncatched(const std::__fs::filesystem::path &pwd);

        bool IsExtensionMatched(const std::__fs::filesystem::path &extension) const;
        ...
        std::__fs::filesystem::path pwd_;
        std::set<std::__fs::filesystem::path> selectedFilenames_;
      ...
      inputNameBuf_(std::make_unique<std::array<char, INPUT_NAME_BUF_SIZE>>())
{
    ...
    SetPwd(std::__fs::filesystem::current_path());
    ...
    const FileBrowser &copyFrom)
{
    ...
    SetTitle(copyFrom.title_, dirname_);
...
inline void ImGui::FileBrowser::SetTitle(std::string title, std::string dirname)
{
...
inline bool ImGui::FileBrowser::SetPwd(const std::__fs::filesystem::path &pwd)
{   
    cout << dirname_ << "\n";
    const filesystem::path newpwd(dirname_);
    
    try
    {
        SetPwdUncatched(newpwd);
        return true;
    }
...
inline const class std::__fs::filesystem::path &ImGui::FileBrowser::GetPwd() const noexcept
{
    return pwd_;
}

inline std::__fs::filesystem::path ImGui::FileBrowser::GetSelected() const
{
...
inline std::vector<std::__fs::filesystem::path>
    ImGui::FileBrowser::GetMultiSelected() const
{
    ...
    std::vector<std::__fs::filesystem::path> ret;
...
inline void ImGui::FileBrowser::SetPwdUncatched(const std::__fs::filesystem::path &pwd)
{
    pwd_ = absolute(pwd);
    UpdateFileRecords();
    selectedFilenames_.clear();
...
inline bool ImGui::FileBrowser::IsExtensionMatched(
    const std::__fs::filesystem::path &_extension) const
{
#ifdef _WIN32
